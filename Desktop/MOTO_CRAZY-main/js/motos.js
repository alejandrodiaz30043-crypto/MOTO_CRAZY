...
function convertirMoneda(valorCOP) {
    if (monedaActual === "USD") {
        return valorCOP * tasaCambioUSD;
    }

    return valorCOP;
}

function formatearMonedaGlobal(valorCOP) {
    const valor = convertirMoneda(valorCOP);

    return new Intl.NumberFormat(
        monedaActual === "USD" ? "en-US" : "es-CO",
        {
            style: "currency",
...
// =========================================================================
// CONVERSOR DE DIVISAS GLOBAL (COP ↔ USD)
// =========================================================================

let monedaActual = "COP";

let tasaCambioUSD = 0.00025;

// Obtener tasa de cambio real
async function actualizarTasaCambio() {
    try {

        const respuesta = await fetch("https://open.er-api.com/v6/latest/COP");

        if (!respuesta.ok) {
            throw new Error("Error obteniendo tasa");
        }
        ...
        console.error(
            "MotoCrazy: usando tasa fija de respaldo",
            error
        );
    }
}

// Convertir COP → moneda seleccionada
function convertirMoneda(valorCOP) {
...
function formatearMonedaGlobal(valorCOP) {
    ...
    const valorConvertido = convertirMoneda(valorCOP);
...
// =========================================================================
// COTIZADOR DE CUOTAS CON CONVERSOR
// =========================================================================
...
function renderInstallmentQuote() {
            ...
            <section>
                <h2>
                    ${Aplicacion.escapeHTML(
                        Aplicacion.t("tituloCuotas")
                    )}
                </h2>

                <p>
                    ${Aplicacion.escapeHTML(
                        Aplicacion.t("cuotasSinMotos")
                    )}
                </p>
            </section>
        `;
    ...
    const selectedId =
        node.dataset.selectedMoto || motos[0].id;
    ...
    const downPayment =
        node.dataset.downPayment || "";

    const term =
        node.dataset.term || "24";

    const fixedRate =
        formatPercent(FIXED_ANNUAL_INTEREST_RATE);
    ...
    node.innerHTML = `
        ...
        <section class="cotizador-cuotas__texto">
            ...
            <p class="ceja">
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("botonCotizarCuotas")
                )}
            </p>
        ...
        <form class="cotizador-cuotas__formulario">
                ...
                >
                    ...
                    ${motos.map(function (producto) {
                        ...
                        return `
                            ...
                            <option
                                value="${Aplicacion.escapeAttribute(
                                    producto.id
                                )}"

                                ${String(producto.id) === String(selected.id)
                                    ? "selected"
                                    : ""}
                            >
            ...
            <label>
                ...
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("cuotaInicialTexto")
                )}
            ...
            <label>
                ...
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("interesMensualTexto")
                )}
        ...
        <section
            id="resultadoCuotas"
            class="cotizador-cuotas__resultado"
        ></section>
...
function updateInstallmentQuote() {
    ...
    const node =
        document.getElementById("cotizadorCuotas");

    const result =
        document.getElementById("resultadoCuotas");

    const motoSelect =
        document.getElementById("motoCuotas");

    const downInput =
        document.getElementById("cuotaInicial");

    const termSelect =
        document.getElementById("plazoCuotas");

    const selectorMoneda =
        document.getElementById("selectorMoneda");
    ...
    const producto =
        Aplicacion.estado.catalogs.motos.find(function (item) {

            return (
                String(item.id) ===
                String(motoSelect.value)
            );
    ...
    const price =
        Math.max(0, Number(producto.precio || 0));
    ...
    const months =
        Math.max(
            1,
            Number(termSelect.value || 1)
        );

    const monthlyRate =
        Math.pow(
            1 + FIXED_ANNUAL_INTEREST_RATE,
            1 / 12
        ) - 1;
    ...
    result.innerHTML = `
        ...
        <section class="cotizador-cuotas__fila">
            ...
            <span>
                ...
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("ivaTexto")
                )}

                (
                    ${Aplicacion.escapeHTML(
                        formatPercent(IVA_RATE)
                    )}
                )
        ...
        <section class="cotizador-cuotas__fila">
            ...
            <span>
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("precioConIvaTexto")
                )}
            </span>

            <strong>
                ${Aplicacion.escapeHTML(
                    formatearMonedaGlobal(priceWithTax)
                )}
            </strong>
        ...
        <section class="cotizador-cuotas__fila">
            ...
            <span>
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("valorFinanciadoTexto")
                )}
            </span>

            <strong>
                ${Aplicacion.escapeHTML(
                    formatearMonedaGlobal(financed)
                )}
            </strong>
        ...
        <section class="cotizador-cuotas__fila cotizador-cuotas__fila--principal">
            ...
            <span>
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("cuotaMensualTexto")
                )}
            </span>

            <strong>
                ${Aplicacion.escapeHTML(
                    formatearMonedaGlobal(monthlyPayment)
                )}
            </strong>
        ...
        <section class="cotizador-cuotas__fila">
            ...
            <span>
                ${Aplicacion.escapeHTML(
                    Aplicacion.t("costoTotalTexto")
                )}
            </span>

            <strong>
                ${Aplicacion.escapeHTML(
                    formatearMonedaGlobal(total)
                )}
            </strong>
